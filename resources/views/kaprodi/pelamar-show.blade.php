            ...
            <div>
                ...
                <div class="space-y-6">
                    @foreach($pelamar->lamarans as $lamaran)
                        @php
                            $wawancara = $allJadwals->where('lowongan_id', $lamaran->lowongan_id)->where('tipe_seleksi', 'wawancara')->first();
                            $micro = $allJadwals->where('lowongan_id', $lamaran->lowongan_id)->where('tipe_seleksi', 'micro_teaching')->first();
                        ...
                        @if($hasWawancaraScore || $hasMicroScore)
                            ...
                            <div>
                                <h4 class="text-xs font-bold text-gray-600 uppercase tracking-wider mb-3">Lamaran: <span class="text-[#8b1515]">{{ $lamaran->lowongan?->nama_posisi ?? '-' }}</span></h4>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    @if($hasWawancaraScore)
                                    <div class="rounded-xl border border-gray-100 p-5 bg-gray-50/50">
                                        <h3 class="text-sm font-black text-[#8b1515] uppercase tracking-widest mb-4 border-b border-gray-200 pb-2">Wawancara</h3>
                                        <div class="space-y-3">
                                            <div class="flex justify-between items-center gap-4"><span class="text-[0.65rem] font-bold text-gray-500 uppercase">Kepribadian & Integritas</span><span class="text-sm font-bold text-gray-800">{{ $wawancara->penilaian->kategori_1 }}</span></div>
                                            <div class="flex justify-between items-center gap-4"><span class="text-[0.65rem] font-bold text-gray-500 uppercase">Visi & Profesionalisme</span><span class="text-sm font-bold text-gray-800">{{ $wawancara->penilaian->kategori_2 }}</span></div>
                                            <div class="flex justify-between items-center gap-4"><span class="text-[0.65rem] font-bold text-gray-500 uppercase">Adaptasi & Kolaborasi</span><span class="text-sm font-bold text-gray-800">{{ $wawancara->penilaian->kategori_3 }}</span></div>
                                            <div class="pt-3 mt-3 border-t border-gray-200 flex justify-between items-center"><span class="text-xs font-black text-gray-800 uppercase">Total Nilai Akhir</span><span class="text-2xl font-black text-[#8b1515]">{{ $wawancara->penilaian->total_nilai }}</span></div>
                                        </div>
                                    </div>
                                    @endif
                                    @if($hasMicroScore)
                                    <div class="rounded-xl border border-gray-100 p-5 bg-gray-50/50">
                                        <h3 class="text-sm font-black text-[#8b1515] uppercase tracking-widest mb-4 border-b border-gray-200 pb-2">Micro Teaching</h3>
                                        <div class="space-y-3">
                                            <div class="flex justify-between items-center gap-4"><span class="text-[0.65rem] font-bold text-gray-500 uppercase">Penguasaan Materi</span><span class="text-sm font-bold text-gray-800">{{ $micro->penilaian->kategori_1 }}</span></div>
                                            <div class="flex justify-between items-center gap-4"><span class="text-[0.65rem] font-bold text-gray-500 uppercase">Keterampilan Pedagogik</span><span class="text-sm font-bold text-gray-800">{{ $micro->penilaian->kategori_2 }}</span></div>
                                            <div class="flex justify-between items-center gap-4"><span class="text-[0.65rem] font-bold text-gray-500 uppercase">Media Pembelajaran</span><span class="text-sm font-bold text-gray-800">{{ $micro->penilaian->kategori_3 }}</span></div>
                                            <div class="pt-3 mt-3 border-t border-gray-200 flex justify-between items-center"><span class="text-xs font-black text-gray-800 uppercase">Total Nilai Akhir</span><span class="text-2xl font-black text-[#8b1515]">{{ $micro->penilaian->total_nilai }}</span></div>
                                        </div>
                                    </div>
                                    @endif
                                </div>
