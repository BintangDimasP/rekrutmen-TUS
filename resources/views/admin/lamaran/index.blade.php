...
<div class="max-w-6xl mx-auto space-y-6"
    ...
    <div class="relative">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 px-5 py-4 pr-[168px]">
            <div class="flex items-center gap-3 flex-wrap">
                    ...
                    <div x-show="statusOpen" x-transition class="absolute top-full left-0 mt-2 w-64 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden" style="display:none;">
                        ...
                        <div class="p-3 space-y-1">
                            <button type="button" @click="statusFilter = statusFilter === 'menunggu' ? '' : 'menunggu'; statusOpen = false" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-left" :class="statusFilter === 'menunggu' ? 'bg-gray-50' : ''">
                                <span class="w-4 h-4 rounded border-2 flex items-center justify-center" :class="statusFilter === 'menunggu' ? 'border-[#8b1515] bg-[#8b1515]' : 'border-gray-300'"><svg x-show="statusFilter === 'menunggu'" class="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg></span>
                            ...
                            <button type="button" @click="statusFilter = statusFilter === 'seleksi_tahap1' ? '' : 'seleksi_tahap1'; statusOpen = false" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-left" :class="statusFilter === 'seleksi_tahap1' ? 'bg-gray-50' : ''">
                                <span class="w-4 h-4 rounded border-2 flex items-center justify-center" :class="statusFilter === 'seleksi_tahap1' ? 'border-[#8b1515] bg-[#8b1515]' : 'border-gray-300'"><svg x-show="statusFilter === 'seleksi_tahap1'" class="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg></span>
                            ...
                            <button type="button" @click="statusFilter = statusFilter === 'seleksi_tahap2' ? '' : 'seleksi_tahap2'; statusOpen = false" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-left" :class="statusFilter === 'seleksi_tahap2' ? 'bg-gray-50' : ''">
                                <span class="w-4 h-4 rounded border-2 flex items-center justify-center" :class="statusFilter === 'seleksi_tahap2' ? 'border-[#8b1515] bg-[#8b1515]' : 'border-gray-300'"><svg x-show="statusFilter === 'seleksi_tahap2'" class="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg></span>
                ...
                {{-- Active filter tag --}}
                <span x-show="statusFilter !== ''" x-transition class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-red-50 border border-red-200 text-xs font-semibold text-[#8b1515]">
                    <span x-text="{menunggu:'Menunggu',seleksi_tahap1:'Seleksi Tahap 1',seleksi_tahap2:'Seleksi Tahap 2',diterima:'Diterima',ditolak:'Ditolak'}[statusFilter]"></span>
                    <button type="button" @click="statusFilter = ''" class="ml-0.5 hover:text-red-800"><svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button>
                </span>
        ...
        {{-- Export Nilai button (outside card, third from right) --}}
        <a href="{{ route('admin.lamaran.exportNilai', $lowongan) }}"
           class="absolute top-0 right-28 h-full w-14 flex items-center justify-center bg-white border-y border-l border-gray-100 text-gray-500 hover:text-gray-700 hover:bg-gray-50 transition-colors" title="Export Rekap Nilai">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
        </a>

        {{-- Export Excel button (outside card, second from right) --}}
        <a href="{{ route('admin.lamaran.export', $lowongan) }}"
           class="absolute top-0 right-14 h-full w-14 flex items-center justify-center bg-white border-y border-l border-gray-100 text-gray-500 hover:text-gray-700 hover:bg-gray-50 transition-colors" title="Export Excel">
            ...
            <table class="w-full text-left border-collapse table-fixed">
                ...
                <tbody class="divide-y divide-gray-100" x-ref="tableBody">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors"
                        ...
                        <td class="py-3 px-5">
                            <div class="text-sm font-semibold text-gray-800 truncate">{{ $lamaran->pelamar->nama }}</div>
                            <div class="text-xs text-gray-400 font-mono mt-0.5 truncate">{{ $lamaran->pelamar->user?->email }}</div>
                        </td>
                        <td class="py-3 px-5">
                            <div class="text-sm text-gray-700 font-medium truncate">{{ $lamaran->pelamar->jenjang }} - {{ $lamaran->pelamar->institusi }}</div>
                            <div class="text-[0.7rem] text-gray-400 uppercase tracking-widest mt-0.5 truncate">{{ $lamaran->pelamar->prodi_pendidikan }}</div>
                        </td>
                        <td class="py-3 px-5">
                            <span class="text-sm text-gray-600 font-mono">{{ $lamaran->pelamar->no_telepon ?? '-' }}</span>
                        </td>
                        <td class="py-3 px-5">
                            <span class="text-sm text-gray-600 truncate">{{ $lamaran->pelamar->user?->email ?? '-' }}</span>
                        </td>
                        <td class="py-3 px-5">
                            @php
                                $statusColors = [
                            ...
                            @endphp
                            <span class="inline-flex px-2.5 py-1 rounded-md text-[0.7rem] font-bold border {{ $statusColors[$lamaran->status] ?? $statusColors['menunggu'] }}">
                        ...
                        <td class="py-3 px-5">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form method="POST" action="{{ route('admin.lamaran.destroy', $lamaran) }}"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" title="Hapus" class="flex items-center justify-center p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors">
