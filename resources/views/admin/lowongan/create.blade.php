...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-7">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Isi seluruh informasi posisi yang akan dibuka untuk pelamar.</p>
        </div>
    </div>

    <form method="POST" action="{{ route('admin.lowongan.store') }}" class="space-y-6">
        @csrf
            ...
            <div class="p-6 space-y-5">
                ...
                <div>
                    ...
                    <select name="prodi_id"
                            class="w-full px-4 py-2.5 rounded-xl bg-gray-50 border @error('prodi_id') border-red-400 @else border-gray-200 @enderror text-sm text-gray-800 focus:outline-none focus:bg-white focus:border-[#8b1515] focus:ring-2 focus:ring-[#8b1515]/15 transition-all">
                    ...
                    @error('prodi_id') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>

                {{-- Status & Tanggal Tutup --}}
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Tanggal Penutupan Pendaftaran <span class="text-red-500">*</span></label>
                        <input type="date" name="tanggal_tutup" value="{{ old('tanggal_tutup') }}"
                               class="w-full px-4 py-2.5 rounded-xl bg-gray-50 border @error('tanggal_tutup') border-red-400 @else border-gray-200 @enderror text-sm text-gray-800 focus:outline-none focus:bg-white focus:border-[#8b1515] focus:ring-2 focus:ring-[#8b1515]/15 transition-all">
                        @error('tanggal_tutup') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                    </div>
                </div>
            ...
            <div class="p-6 grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="jenjang_minimal"
                            class="w-full px-4 py-2.5 rounded-xl bg-gray-50 border @error('jenjang_minimal') border-red-400 @else border-gray-200 @enderror text-sm text-gray-800 focus:outline-none focus:bg-white focus:border-[#8b1515] focus:ring-2 focus:ring-[#8b1515]/15 transition-all">
                        <option value="">— Pilih Jenjang —</option>
                        @foreach(['D3', 'S1', 'S2', 'S3'] as $j)
                            <option value="{{ $j }}" {{ old('jenjang_minimal') === $j ? 'selected' : '' }}>{{ $j }}</option>
                        @endforeach
                    </select>
                    @error('jenjang_minimal') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="minimal_ipk" value="{{ old('minimal_ipk', '3.00') }}"
                           ...
                           class="w-full px-4 py-2.5 rounded-xl bg-gray-50 border @error('minimal_ipk') border-red-400 @else border-gray-200 @enderror text-sm text-gray-800 focus:outline-none focus:bg-white focus:border-[#8b1515] focus:ring-2 focus:ring-[#8b1515]/15 transition-all">
                    @error('minimal_ipk') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex items-center gap-3">
                        <input type="number" name="kuota" value="{{ old('kuota', 1) }}" min="1" placeholder="cth: 5"
                               class="w-40 px-4 py-2.5 rounded-xl bg-gray-50 border @error('kuota') border-red-400 @else border-gray-200 @enderror text-sm text-gray-800 focus:outline-none focus:bg-white focus:border-[#8b1515] focus:ring-2 focus:ring-[#8b1515]/15 transition-all">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="bg-[#8b1515] px-6 py-4 flex items-center justify-between">
                ...
                <span class="text-white/70 text-xs">Sudah diisi secara otomatis — dapat diedit</span>
            </div>
            <div class="p-6">
                <p class="text-xs text-blue-700 bg-blue-50 rounded-lg px-3 py-2 mb-4 leading-relaxed">
                    Deskripsi berikut sudah diisi template standar dokumen TUS. Kamu dapat mengedit atau menambahkan informasi di bawah ini.
                </p>
                <textarea name="deskripsi" rows="18"
                          class="w-full px-4 py-3 rounded-xl bg-gray-50 border border-gray-200 text-sm text-gray-800 font-mono leading-relaxed focus:outline-none focus:bg-white focus:border-[#8b1515] focus:ring-2 focus:ring-[#8b1515]/15 transition-all resize-y">{{ old('deskripsi', $defaultDeskripsi) }}</textarea>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pb-6">
            ...
            <button type="submit" name="status" value="draft"
                    class="px-6 py-2.5 text-sm font-semibold text-gray-700 bg-amber-100 hover:bg-amber-200 rounded-xl transition-colors shadow-sm">
                💾 Simpan sebagai Draft
            </button>
            <button type="submit" name="status" value="aktif"
                    class="px-6 py-2.5 text-sm font-semibold text-white bg-[#8b1515] hover:bg-red-900 rounded-xl shadow-md transition-colors">
                🚀 Terbitkan Sekarang
            </button>
        </div>
