    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        ...
        <div class="flex gap-2 items-center">
            ...
            @elseif($lowongan->status === 'draft')
                <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold bg-amber-50 text-amber-700 border border-amber-200">
                    DRAFT
                </span>
            @else
                <span class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold bg-gray-50 text-gray-600 border border-gray-200">
                    DITUTUP
                </span>
            @endif

            <a href="{{ route('admin.lowongan.beritaAcara', $lowongan) }}" target="_blank"
               class="inline-flex items-center px-4 py-2 rounded-xl text-xs font-bold bg-[#8b1515] hover:bg-red-900 text-white shadow-sm shadow-red-900/20 transition-all">
                Cetak Berita Acara
            </a>
        </div>
    </div>
            ...
            <table class="w-full text-left border-collapse table-fixed">
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="py-3 px-5">
                            <div class="flex items-center justify-end gap-3 text-[0.8rem]">
                                <a href="{{ route('admin.lamaran.show', $lamaran) }}" class="text-blue-600 hover:text-blue-800 hover:underline transition-colors font-medium">Detail</a>
                                <a href="{{ route('admin.lamaran.cetak', $lamaran) }}" target="_blank" class="text-green-600 hover:text-green-800 hover:underline transition-colors font-medium">Cetak</a>
                                <form method="POST" action="{{ route('admin.lamaran.destroy', $lamaran) }}"
                                    ...
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-red-600 hover:text-red-800 hover:underline transition-colors font-medium">Hapus</button>
                                </form>
                            </div>
                        </td>
                    </tr>
