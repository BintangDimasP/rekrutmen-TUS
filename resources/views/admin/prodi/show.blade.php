        ...
        <div class="relative">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 px-5 py-4 pr-20">
                <div class="flex items-center gap-3 flex-wrap">
                    ...
                    <div class="relative" x-data="{ statusOpen: false }" @click.outside="statusOpen = false">
                        ...
                        <div x-show="statusOpen" x-transition class="absolute top-full left-0 mt-2 w-56 bg-white border border-gray-200 rounded-xl shadow-xl z-50 overflow-hidden" style="display:none;">
                            ...
                            <div class="p-3 space-y-1">
                                <button type="button" @click="statusFilter = statusFilter === 'penguji' ? '' : 'penguji'; statusOpen = false" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-left" :class="statusFilter === 'penguji' ? 'bg-gray-50' : ''">
                                    <span class="w-4 h-4 rounded border-2 flex items-center justify-center" :class="statusFilter === 'penguji' ? 'border-[#8b1515] bg-[#8b1515]' : 'border-gray-300'"><svg x-show="statusFilter === 'penguji'" class="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg></span>
                                    <span class="text-sm font-medium text-blue-600">Penguji</span>
                                </button>
                                <button type="button" @click="statusFilter = statusFilter === 'kaprodi' ? '' : 'kaprodi'; statusOpen = false" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 text-left" :class="statusFilter === 'kaprodi' ? 'bg-gray-50' : ''">
                                    <span class="w-4 h-4 rounded border-2 flex items-center justify-center" :class="statusFilter === 'kaprodi' ? 'border-[#8b1515] bg-[#8b1515]' : 'border-gray-300'"><svg x-show="statusFilter === 'kaprodi'" class="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg></span>
                                    <span class="text-sm font-medium text-amber-600">Kaprodi</span>
                                </button>
                            </div>
                        </div>
                    </div>
                    ...
                    {{-- Active tag --}}
                    <template x-if="statusFilter === 'penguji'"><span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-blue-50 border border-blue-200 text-xs font-semibold text-blue-700">Penguji <button type="button" @click="statusFilter = ''" class="ml-0.5 hover:text-blue-900"><svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button></span></template>
                    <template x-if="statusFilter === 'kaprodi'"><span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md bg-amber-50 border border-amber-200 text-xs font-semibold text-amber-700">Kaprodi <button type="button" @click="statusFilter = ''" class="ml-0.5 hover:text-amber-900"><svg class="w-3 h-3" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button></span></template>

                    {{-- Clear --}}
                    <button x-show="statusFilter !== '' || search !== ''" x-transition type="button" @click="statusFilter = ''; search = ''" class="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-gray-500 hover:text-red-600 transition-colors">
                    ...
                    <button type="button" @click="openImportModal = true"
                            class="inline-flex items-center justify-center w-10 h-10 rounded-lg border border-gray-200 bg-white text-gray-600 hover:border-gray-300 hover:text-gray-800 transition-all" title="Import Excel">
                        <svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                    </button>
                </div>
            </div>
