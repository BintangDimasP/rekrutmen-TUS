...
class ProfilController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $request->validate([
            // Data Diri
            'email' => ['required', 'email', 'max:255', 'unique:users,email,'.auth()->id(), new NotDosenInternalDomain],
            'nik' => [
                'required', 'string', 'digits:16',
                function ($attr, $value, $fail) use ($pelamar) {
                    $exists = \App\Models\Pelamar::all()
                        ->first(fn($p) => $p->nik === $value && $p->id !== $pelamar->id);
                    if ($exists) $fail('NIK sudah terdaftar oleh akun lain.');
                },
            ],
            ...
            'no_telepon' => [
                ...
                function ($attr, $value, $fail) use ($pelamar) {
                    $exists = \App\Models\Pelamar::all()
                        ->first(fn($p) => $p->no_telepon === $value && $p->id !== $pelamar->id);
                    if ($exists) $fail('No. telepon sudah terdaftar oleh akun lain.');
                },
            ],
